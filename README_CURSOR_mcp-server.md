...
## 0) What this repo is
后端 Node/Express 服务，部署到 **Google Cloud Run（服务：mcp-core）**。职责：对接 Supabase（Auth/Storage）、提供最小 API（含 `POST /generate-card`），并启用**严格 CORS 白名单**。
...
- 关键路由：
  ...
  - `POST /generate-card` → 接收 `image/png`，写入 Supabase Storage 公有桶 `cards`，返回 public URL
...
- Vercel 预览域：`https://app-web-tawny-zeta.vercel.app`（或通过 `CORS_ORIGINS` 追加）
...
- 预检：`OPTIONS` 直接 `204`
- **凭据策略**：MVP 采用 **Bearer Token**（`Authorization: Bearer <token>`），**不**启用 `Allow-Credentials`。
...
| `CORS_ORIGINS` | `https://preview-1.vercel.app,https://preview-2.vercel.app` | Cloud Run | 追加 CORS 白名单 |
...
**预检（OPTIONS）**
```bash
curl -i -X OPTIONS "https://mcp-core-70953787995.us-central1.run.app/health"   -H "Origin: https://staging.useweeknight.com"   -H "Access-Control-Request-Method: POST"   -H "Access-Control-Request-Headers: Content-Type, Authorization"
```
**实际请求（GET）**
```bash
curl -i "https://mcp-core-70953787995.us-central1.run.app/health"   -H "Origin: https://app.useweeknight.com"
```
...
- 如需新增 Tonight / 菜谱相关 API 或 MCP 工具，务必以根目录 `菜谱库_产品说明_v1.0.md` 与 `schema.sql` 里的 recipe / recipe_step / recipe_media / nutrition_snapshot 等表为唯一数据源，不要在后端发明额外字段名。
